# Privacy Policy

**Last updated:** September 2026

**Clip Squeezer** is designed with a strict privacy-first architecture. We believe that personal video utility tasks should never compromise your privacy or require handing over your media to cloud servers.

---

### 1. 100% Local Execution
All video and audio processing — including compression, format conversion, trimming, audio extraction, and media probing — runs entirely on your computer using bundled local binaries (FFmpeg & FFprobe). 

Your media files never leave your device.

### 2. No Network Transmission or Cloud Storage
- The application does not upload your files, video clips, audio tracks, or file metadata to any external server or cloud service.
- The application works completely offline. No active internet connection is required to use any of its features.

### 3. Zero Telemetry, Analytics, or Tracking
- We do not collect, store, or transmit analytics, usage statistics, device fingerprints, or crash reports.
- There are no tracking scripts, third-party SDKs, telemetry beacons, or advertising cookies bundled with the software.

### 4. Non-Destructive File Handling
The application treats your original media as read-only:
- Your original files are never overwritten, altered, or deleted.
- All operations produce a new, separate output file.

### 5. Local State and Preferences
Any application state (such as recent activity history or your preferred destination directory) is saved strictly on your local machine using standard local storage mechanisms. You can clear your activity history at any time with the "Clear History" button.

### 6. Open Source and Verifiable
The source code for this utility is open and auditable. Anyone can inspect the code to verify that no network requests or background data collections are performed.

---

### Contact & Questions
If you have any questions or feedback regarding this policy, please open an issue on the project's GitHub repository:  
https://github.com/rjk/clip-squeezer
